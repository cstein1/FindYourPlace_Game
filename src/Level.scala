...
class Level(intMaze: Array[Array[Int]], private var chars: List[Entity]) extends Serializable {
  ...
  private val npcsList = MutableList[NPC]()
  private val psList = MutableList[Player]()
  def npcs = {
    npcList.clear()
    pList.clear()
    for (n <- chars) {
      n match {
        case np: NPC => npcList += np
        case p: Player => pList += p
      }
    }
  }
  def pList = psList
  ...
  def breadthFirstShortestPath(y: Int, x: Int, ey: Int, ex: Int): Int = {
    ...
    def happySpot(cx: Int, cy: Int): Boolean = {
      cx >= 0 && cy >= 0 && cx < maze.length && cy < maze(0).length && maze(cx)(cy).canPass &&
        !visited((cx, cy))
    }
    npcs
